...
export function JourneyProgress() {
  ...
  const { lang } = useLang()
  const ko = lang === "ko"
  const active = demoJourney.stage === "request-ready" || demoJourney.stage === "presentation-created"
    ? 0
    ...
    <section className="overflow-hidden rounded-3xl border border-border bg-card shadow-[0_10px_30px_rgba(28,24,19,0.045)]">
      <div className="flex items-center justify-between gap-3 px-4 pb-3 pt-4">
        <div>
          ...
          <p className="mt-0.5 text-[13px] font-bold">{complete ? (ko ? "하나의 영수증으로 전체 흐름 완료" : "One receipt completed the full journey") : (ko ? "다음 단계까지 이어서 시연" : "Continue the same transaction")}</p>
        </div>
        {complete && <button type="button" onClick={resetDemoJourney} aria-label={ko ? "데모 초기화" : "Reset demo"} className="pressable grid h-9 w-9 flex-shrink-0 place-items-center rounded-full bg-secondary text-muted-foreground"><RotateCcw className="h-4 w-4" /></button>}
      </div>
      ...
      <Link href={href} className="pressable flex min-h-11 items-center justify-between border-t border-border bg-surface-2 px-4 text-[11px] font-bold">
        <span>{complete ? (ko ? "공통 영수증 보기" : "View shared receipt") : (ko ? "이 거래 계속하기" : "Continue this transaction")}</span>
