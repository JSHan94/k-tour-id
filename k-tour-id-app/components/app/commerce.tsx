...
export function EditorialFeature({ item, voucher }: { item: MarketplaceItem; voucher?: Voucher }) {
  const { lang } = useLang()
  ...
  return (
    <Link href={`/explore/${item.id}`} className="pressable group block">
      <div className="relative aspect-[16/10] overflow-hidden rounded-[24px] bg-secondary">
        ...
        {item.integrationMode === "simulated" && <span className="absolute left-4 top-4 rounded-full bg-background/88 px-3 py-1.5 text-[11px] font-semibold text-foreground backdrop-blur">{lang === "ko" ? "데모 예시" : "Demo concept"}</span>}
      </div>
      <div className="pt-4">
        <div className="flex items-start justify-between gap-4">
          <div className="min-w-0"><h3 className="font-display text-balance text-[24px] font-semibold leading-[1.28] tracking-[-0.025em]">{item.title[lang]}</h3><p className="mt-2 flex items-center gap-1.5 text-[12px] text-muted-foreground"><CalendarDays className="h-3.5 w-3.5" />{item.availability[lang]} · {item.location[lang]}</p></div>
...
export function ServiceRow({ item, voucher }: { item: MarketplaceItem; voucher?: Voucher }) {
  const { lang } = useLang()
  const amount = price(item, voucher)
  return (
    <Link href={`/explore/${item.id}`} className="pressable flex min-h-[128px] items-center gap-4 py-4">
      ...
      <span className="min-w-0 flex-1">
        ...
        <span className="mt-1.5 flex items-center gap-1 text-[12px] text-muted-foreground"><MapPin className="h-3 w-3" />{item.location[lang]}</span>
