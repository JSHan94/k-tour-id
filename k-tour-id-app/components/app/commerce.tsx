...
export function EditorialFeature({ item, voucher, proximity, reason }: { item: MarketplaceItem; voucher?: Voucher; proximity?: string; reason?: string }) {
  const { lang } = useLang()
  ...
  return (
    <Link href={`/explore/${item.id}`} className="pressable group block">
      ...
      <div className="pt-4">
        ...
        {reason && <p title={reason} className="mt-3 truncate text-[13px] font-medium text-primary"><span className="font-semibold">{lang === "ko" ? "추천 이유" : "Why this pick"}</span> · {reason}</p>}
