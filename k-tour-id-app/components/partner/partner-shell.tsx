...
import { BadgeCheck, Building2, CircleDot, Landmark, ScanLine, ShieldCheck } from "lucide-react"
...
export function EnvironmentBadge({ kind, className }: { kind: EnvironmentKind; className?: string }) {
  return (
    <span
      className={cn(
        "inline-flex items-center gap-1.5 rounded-full border px-2.5 py-1 text-[10px] font-extrabold tracking-[0.12em]",
        ENV_STYLES[kind],
        className,
      )}
    >
...
const NAV = [
  { href: "/partner/verify", label: "Verifier", sub: "VP request & policy", icon: ScanLine },
  { href: "/partner/settlements", label: "Settlements", sub: "Payment & anchor", icon: Landmark },
] as const
    ...
    <div className="min-h-screen bg-background text-foreground">
      <div className="mx-auto grid min-h-screen w-full max-w-[1480px] lg:grid-cols-[260px_minmax(0,1fr)]">
        <aside className="hidden border-r border-border bg-card/70 px-5 py-6 lg:flex lg:flex-col">
              ...
              return (
                ...
                >
                  ...
                  <span className="min-w-0">
                    ...
                    <span className={cn("block text-[10px]", active ? "text-white/55" : "text-muted-foreground")}>{sub}</span>
                  </span>
                </Link>
              )
            })}
          ...
          <div className="mt-auto rounded-2xl border border-border bg-card p-3.5">
            <div className="flex items-center gap-2">
              <ShieldCheck className="h-4 w-4 text-success" />
              <p className="text-[12px] font-bold">Privacy edge active</p>
            </div>
            <p className="mt-1.5 text-[10.5px] leading-relaxed text-muted-foreground">
              This console stores policy results and event references, not raw identity documents.
            </p>
          </div>
        </aside>
  ...
  return (
    <div className="flex items-center gap-3">
      ...
      <div>
        ...
        <p className="text-[10px] font-bold uppercase tracking-[0.14em] text-muted-foreground">Partner console</p>
      </div>
    </div>
  )
}
    ...
    <section className={cn("rounded-3xl border border-border bg-card shadow-[0_12px_38px_rgba(28,24,19,0.05)]", className)}>
      {(title || eyebrow || action) && (
        <div className="flex items-start justify-between gap-4 border-b border-border px-5 py-4 sm:px-6">
          <div>
            {eyebrow && <p className="text-[10px] font-extrabold uppercase tracking-[0.14em] text-primary">{eyebrow}</p>}
...
export function EnvironmentLegend() {
  return (
    <div className="flex flex-wrap items-center gap-2 rounded-2xl border border-border bg-card p-2.5">
      <EnvironmentBadge kind="LIVE" />
      <span className="mr-2 text-[10px] text-muted-foreground">production-connected</span>
      <EnvironmentBadge kind="SANDBOX" />
      <span className="mr-2 text-[10px] text-muted-foreground">test endpoint</span>
      <EnvironmentBadge kind="SIMULATED" />
      <span className="text-[10px] text-muted-foreground">local demo event</span>
    </div>
  )
}
...
export function VerifiedMark({ label = "Verified" }: { label?: string }) {
  return (
    <span className="inline-flex items-center gap-1 rounded-full bg-success-surface px-2.5 py-1 text-[11px] font-bold text-success">
