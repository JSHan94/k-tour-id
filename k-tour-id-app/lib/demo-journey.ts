...
export function demoClaimValue(key: DemoClaimKey, userType: UserType | null, voucher?: Voucher, capsule?: KPassCapsule | null, serviceEligible = false): boolean {
  ...
  return voucher?.status === "available" && new Date(voucher.expiresAt).getTime() > Date.now()
}
...
export function voucherMatchesPurchase(
  ...
  if (voucher.status !== "available" || new Date(voucher.expiresAt).getTime() <= Date.now()) return false
  if (voucher.redemption !== "single-use" && voucher.redemption !== "stored-value") return false
