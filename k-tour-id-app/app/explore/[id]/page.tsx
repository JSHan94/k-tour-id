...
import type { CommerceEntryContext } from "@/lib/types"
...
export default function ProductDetailPage() {
  ...
  const returnTripVoucher = vouchers.find((candidate) => candidate.funding === "user-converted" && candidate.itemId === item?.id && isVoucherAvailable(candidate))
  const voucher = isVoucherAvailable(configuredVoucher) ? configuredVoucher : returnTripVoucher ?? configuredVoucher
  const [optionId, setOptionId] = useState(item?.options.find((option) => option.available)?.id ?? "")
  const [useBenefit, setUseBenefit] = useState(isVoucherAvailable(voucher))
  ...
  const [entryContext, setEntryContext] = useState<CommerceEntryContext | null>(null)
  ...
  useEffect(() => {
    ...
    const contextLabel = query.get("contextLabel") ?? ""
    ...
    setContextQuery(preserved.toString())
    if (contextId && returnTo.startsWith("/")) {
      const nextContext = {
        contextId,
      ...
      try { sessionStorage.setItem(`k-tour-entry-context:${params.id}`, JSON.stringify(nextContext)) } catch { /* keep current navigation context */ }
    } else if (query.get("from") === "map") setBackHref("/")
    ...
    setUseBenefit(requestedBenefit === "0" ? false : isVoucherAvailable(voucher))
  ...
  const personaEligible = !!session.userType && item.eligibleUserTypes.includes(session.userType)
  const credentialUnavailable = !isCredentialUsable(session.capsule)
  const eligible = personaEligible && !credentialUnavailable
  ...
  const detailReturnHref = `/explore/${item.id}?${new URLSearchParams({ ...(backHref === "/" ? { from: "map" } : {}), ...(contextQuery ? Object.fromEntries(new URLSearchParams(contextQuery)) : {}) }).toString()}`
  ...
  const confirm = async () => {
    ...
    setErrorCode("")
    try {
      const result = await purchaseServiceItem({ itemId: item.id, optionId: option.id, useBenefit, deliveryAddress, entryContext: entryContext ?? undefined })
    ...
    <PhoneFrame>
      ...
      <main className="pb-8">
        ...
        <section className="px-6 pt-6">
          <p className={cn("flex items-center gap-1.5 text-[12px] font-semibold", eligible ? "text-success" : "text-muted-foreground")}><BadgeCheck className="h-4 w-4" />{credentialUnavailable ? (ko ? "K-Tour ID 갱신 후 이용 가능" : "Available after renewing K-Tour ID") : eligible ? (ko ? "내 K-Tour ID로 이용 가능" : "Available with your K-Tour ID") : (ko ? "현재 K-Tour ID의 이용 대상이 아닌 상품" : "This item is not available for your K-Tour ID")}</p>
        ...
        <section className="mt-9 px-6">
          <p className="text-[13px] font-semibold text-primary">01 · {ko ? "일정·옵션" : "Date & option"}</p>
        ...
        <section className="mt-9 px-6">
          <p className="text-[13px] font-semibold text-primary">02 · {ko ? "혜택·최종가" : "Benefit & final price"}</p>
          <div className="mt-3 border-y border-foreground/10 py-5">
            {credentialUnavailable ? <p className="text-[13px] leading-6 text-muted-foreground">{ko ? "K-Tour ID가 만료되었거나 사용할 수 없는 상태예요. 갱신을 마치면 이 상품으로 돌아와 혜택과 최종가를 확인할 수 있어요." : "Your K-Tour ID is expired or unavailable. Renew it, then return to review this benefit and final price."}</p> : !personaEligible ? <p className="text-[13px] leading-6 text-muted-foreground">{ko ? "이 상품은 전용 프로그램으로 운영되어 현재 K-Tour ID로는 구매할 수 없어요. 이용 가능한 상품에서 혜택과 최종가를 확인해 주세요." : "This is a dedicated-program item and cannot be purchased with your current K-Tour ID. Open an available item to review benefits and final price."}</p> : <>{voucher ? <button type="button" aria-pressed={useBenefit} disabled={!isVoucherAvailable(voucher)} onClick={() => setUseBenefit((current) => !current)} className="pressable flex w-full items-center gap-3 text-left disabled:opacity-55"><span className={cn("grid h-10 w-10 place-items-center rounded-full", useBenefit ? "bg-success text-white" : "bg-secondary text-muted-foreground")}><ShieldCheck className="h-5 w-5" /></span><span className="min-w-0 flex-1"><strong className="block text-[14px]">{voucher.funding === "user-converted" ? (ko ? `재방문 바우처 잔액 ₩${voucher.valueKRW.toLocaleString()}` : `₩${voucher.valueKRW.toLocaleString()} return-trip voucher`) : (ko ? `K-Tour ID ₩${voucher.valueKRW.toLocaleString()} 혜택` : `₩${voucher.valueKRW.toLocaleString()} K-Tour ID benefit`)}</strong><span className="mt-1 block text-[12px] text-muted-foreground">{isVoucherAvailable(voucher) ? (ko ? "선택한 주문에 적용 가능" : "Available for this order") : (ko ? "이미 사용했거나 만료된 혜택" : "Already used or expired")}</span></span><span className={cn("grid h-6 w-6 place-items-center rounded-full border", useBenefit ? "border-success bg-success text-white" : "border-foreground/20")}>{useBenefit && <Check className="h-3.5 w-3.5" />}</span></button> : <p className="text-[13px] text-muted-foreground">{ko ? "이 상품은 별도 할인 없이 정상가로 이용할 수 있어요." : "This item is available at the regular price."}</p>}<div className="mt-5 flex items-end justify-between border-t border-foreground/10 pt-5"><div><p className="text-[13px] text-muted-foreground">{ko ? "최종 결제 금액" : "Amount due"}</p>{discount > 0 && voucher?.funding !== "user-converted" && <p className="tabular mt-1 text-[12px] text-muted-foreground line-through">₩{gross.toLocaleString()}</p>}</div><div className="text-right"><strong className="font-display tabular text-[35px] font-semibold tracking-[-0.04em]">₩{final.toLocaleString()}</strong>{discount > 0 && <p className="mt-1 text-[12px] font-semibold text-success">{voucher?.funding === "user-converted" ? (ko ? `바우처 ₩${discount.toLocaleString()} 사용` : `₩${discount.toLocaleString()} voucher used`) : (ko ? `₩${discount.toLocaleString()} 절약` : `₩${discount.toLocaleString()} saved`)}</p>}</div></div></>}
          </div>
        </section>
      ...
      {checkoutOpen && option && <CommerceCheckoutSheet item={item} option={option} voucher={voucher} useBenefit={useBenefit} balanceKRW={session.wallet.balanceKRW} busy={busy} error={error} errorCode={errorCode} deliveryAddress={deliveryAddress} returnQuery={contextQuery} onDeliveryAddressChange={setDeliveryAddress} onClose={() => { if (!busy) { setCheckoutOpen(false); setError(""); setErrorCode("") } }} onConfirm={confirm} />}
    </PhoneFrame>
  )
}
