...
test.describe("ONDO B map truth and failure boundary", () => {
  ...
  test("city cards and the compact ONDO temperature key keep official records separate from curated signals", async ({ page }) => {
    ...
    await expect(seoul).toHaveAttribute("aria-label", /Seoul · Place temperature .* · Open map/)
    ...
    await language.click()
    await expect(seoul).toHaveAttribute("aria-label", /서울 · 장소 온도 .* · 지도 열기/)
    ...
    await expect(key).toHaveAttribute("aria-label", "Place temperature · place groups. Outlined numbers group nearby places. Small dots are individual places. Curated visit signals, not live crowding or official LOCALDATA facts.")
