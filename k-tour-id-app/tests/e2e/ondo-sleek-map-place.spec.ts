...
test.describe("SLEEK-R1 map and place closure", () => {
  test("official city and editorial-region semantics stay visible, sans-serif, and unclipped across six target widths", async ({ page }) => {
    ...
    for (const viewport of VIEWPORTS) {
      ...
      await expect(seoul.locator("[data-pulse-city-status]")).toHaveAttribute("data-pulse-city-status", "active")
      await expect(busan.locator("[data-pulse-city-status]")).toHaveAttribute("data-pulse-city-status", "growing")
      ...
      await expect(nation.getByTestId("ondo-b-city-truth-legend")).toHaveAttribute("data-official-count", "400")
  ...
  test("360px place detail keeps the visual Pulse, decisions, and progressive source evidence unclipped", async ({ page }, testInfo) => {
    ...
    await expect(pulse.locator("summary")).toHaveAccessibleName("Pulse 91 · PEAK")
