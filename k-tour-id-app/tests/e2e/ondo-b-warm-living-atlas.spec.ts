...
test.describe("Warm Living Atlas personal journey", () => {
  ...
  test("JA mobile personal surfaces use a trip-first and native-group hierarchy", async ({ page }) => {
    ...
    await expect(my.getByTestId("ondo-b-saved-entry")).toBeVisible()
    await expect(my.locator("[data-memory-venue-card]").first()).toBeVisible()
    ...
    await expect(page.getByTestId("settings-language-control").getByRole("radio")).toHaveCount(3)
    await page.getByRole("dialog", { name: "言語", exact: true }).getByRole("button", { name: "閉じる", exact: true }).click()
    await shot(page, "ja-390-settings")
    await page.getByTestId("nav-id").click()
    await expect(page.getByTestId("wallet-display-equivalent")).toHaveText("—")
    await expect(page.getByTestId("wallet-test-balance")).toContainText("設定")
  ...
  test("post-audit responsive priorities keep decisions ahead of editorial imagery", async ({ browser }) => {
    ...
    await desktop.goto("/", { waitUntil: "domcontentloaded" })
    ...
    await desktopEmpty.goto("/", { waitUntil: "domcontentloaded" })
    await desktopEmpty.getByTestId("nav-my").click()
    await expect(desktopEmpty.getByTestId("my-korea-map-memory")).toBeInViewport()
    await expect(desktopEmpty.getByTestId("my-korea-empty-explore")).toBeInViewport()
    await expect(desktopEmpty.getByTestId("ondo-b-saved-entry")).toHaveCount(0)
    ...
    await landscape.goto("/", { waitUntil: "domcontentloaded" })
    await landscape.getByTestId("nav-my").click()
    await expect(landscape.getByTestId("my-korea-map-memory")).toBeInViewport()
    await expect(landscape.getByTestId("my-korea-empty-explore")).toBeInViewport()
    await expect(landscape.getByTestId("my-korea-planned")).toHaveCount(0)
    ...
    for (const locale of ["en", "ko", "ja"] as const) {
      ...
      await narrow.goto("/", { waitUntil: "domcontentloaded" })
      await narrow.getByTestId("nav-my").click()
      await expect(narrow.getByTestId("ondo-b-my-korea-entry")).toHaveAttribute("data-empty-journey", "true")
      const explore = narrow.getByTestId("my-korea-empty-explore")
    ...
    await languagePage.goto("/", { waitUntil: "domcontentloaded" })
