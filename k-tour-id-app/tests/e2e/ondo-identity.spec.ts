...
test("ID defaults to Guest and unverified checks", async ({ page }) => {
  ...
  await expect(identity).toContainText("Identity check not completed")
  await expect(identity).not.toContainText("Identity check complete · Simulated")
})
...
test("simulated person proof never implies age or payment KYC", async ({ page }) => {
  ...
  await page.getByRole("button", { name: "ID", exact: true }).click()
  const identity = page.getByTestId("ondo-identity-entry")
  await expect(identity).toContainText("Identity check complete · Simulated")
