...
test("canonical place opens a separated offer and both cancel and success return to the exact place", async ({ page }) => {
  ...
  let commerce = page.getByTestId("ondo-b-id-wallet-commerce")
  await expect(commerce).toHaveAttribute("data-origin-venue-id", venueId!)
  await expect(commerce).toHaveRole("dialog")
  await commerce.getByTestId("commerce-outcome-cancelled").click()
  await commerce.getByTestId("commerce-pay").click()
  await expect(commerce).toHaveAttribute("data-payment-state", "CANCELLED")
  await expect(commerce.locator("[data-operation-kind='PAYMENT']")).toHaveCount(0)
  await commerce.getByTestId("commerce-close").click()
  await expect(page.getByTestId("canonical-place-overlay")).toHaveAttribute("data-venue-id", venueId!)
  await expect(page.getByTestId("canonical-demo-meal-offer-open")).toBeFocused()

  await page.getByTestId("canonical-demo-meal-offer-open").click()
  commerce = page.getByTestId("ondo-b-id-wallet-commerce")
  await commerce.getByTestId("commerce-ai-preview").click()
  await expect(commerce.getByTestId("commerce-ai-recommendation")).toHaveAttribute("data-ai-state", "RECOMMENDED")
  await commerce.getByTestId("commerce-ai-accept").click()
  await commerce.getByTestId("commerce-outcome-success").click()
  await commerce.getByTestId("commerce-pay").dblclick()
  await expect(commerce).toHaveAttribute("data-payment-state", "SUCCESS")
  await expect(commerce.getByTestId("commerce-ookrw-balance")).toHaveAttribute("data-balance", "41")
  await expect(commerce.getByTestId("commerce-receipt")).toContainText("ONDO-LOCAL-20260825-001")
  await commerce.getByTestId("commerce-refund").click()
  await expect(commerce).toHaveAttribute("data-payment-state", "REFUNDED")
  await expect(commerce.getByTestId("commerce-ookrw-balance")).toHaveAttribute("data-balance", "60")
  await expect(commerce.getByTestId("commerce-voucher")).toHaveAttribute("data-voucher-state", "AVAILABLE")
  await commerce.getByTestId("commerce-close").click()
  await expect(page.getByTestId("canonical-place-overlay")).toHaveAttribute("data-venue-id", venueId!)
  await expect(page.getByTestId("canonical-demo-meal-offer-open")).toBeFocused()
})
