...
test.describe("K-Tour ID unified deployment brand", () => {
  test("the public root and shared metadata use the same K-Tour ID brand", async ({ page }) => {
    ...
    await expect(page.locator('meta[property="og:image"]')).toHaveAttribute("content", /\/og-ktour-korea-v3\.png$/)
    await expect(page.locator('meta[name="twitter:image"]')).toHaveAttribute("content", /\/og-ktour-korea-v3\.png$/)
  ...
  test("an allowed deployment host owns its canonical and social image URLs", async ({ request, baseURL }) => {
    ...
    expect(html).toContain(`property="og:image" content="${expectedOrigin}/og-ktour-korea-v3.png"`)
    expect(html).toContain(`name="twitter:image" content="${expectedOrigin}/og-ktour-korea-v3.png"`)
