...
test.describe("ONDO B production discovery URL canonicalization", () => {
  for (const locale of ["en", "ko"] as const) {
    test(`strips every non-discovery key and hash while preserving ${locale.toUpperCase()} discovery history`, async ({ page }) => {
      ...
      await page.goto(`/ondo-b?city=seoul&view=list&category=korean&q=${encodeURIComponent("로바")}&qa=1&scenario=save-failed&profile=resident&token=secret-token&email=traveler%40example.com&legacy=1&heat=signal#private-fragment`, { waitUntil: "domcontentloaded" })
      ...
      expect(initial.params).toEqual({ city: "seoul", view: "list", q: "로바", category: "korean" })
      expect(initial.discovery).toMatchObject({ level: "city", city: "seoul", view: "list", query: "로바", category: "korean" })
      ...
      expect(peek.params).toMatchObject({ city: "seoul", view: "list", q: "로바", category: "korean" })
      expect(peek.discovery).toMatchObject({ level: "peek", city: "seoul", view: "list", query: "로바", category: "korean" })
      ...
      expect(reloaded.discovery).toMatchObject({ level: "city", city: "seoul", view: "list", query: "로바", category: "korean" })
    })
  }
})
