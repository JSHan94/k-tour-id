    ...
    test(`${locale.toUpperCase()} nation and city use one semantic heading with explicit official/editorial source boundaries at all six viewports`, async ({ page }) => {
      for (const viewport of VIEWPORTS) {
        await test.step(`${viewport.id} nation`, async () => {
          ...
          await expect(seoul.locator("[data-region-kind-label]")).toHaveText(locale === "ko" ? "먹거리 지도" : "Food map")
          await expect(busan.locator("[data-region-kind-label]")).toHaveText(locale === "ko" ? "먹거리 지도" : "Food map")
          await expect(jeju.locator("[data-region-kind-label]")).toHaveText(locale === "ko" ? "여행 아이디어" : "Travel ideas")
