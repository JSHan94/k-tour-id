...
test.describe("ONDO B official-record discovery and external-map boundary", () => {
  ...
  test("B-TRUTH-PLACE official-source place detail leaves unsupported fields unknown", async ({ page }) => {
    ...
    const unknowns = detail.getByTestId("canonical-place-details-to-check")
    await expect(unknowns).toContainText("Before you go")
    await expect(unknowns.locator("[data-fact-key]" )).toHaveCount(4)
    for (const label of ["Current opening hours", "Foreign-issued card support", "Menu and prices", "English-language support"]) {
      await expect(unknowns.getByRole("button", { name: new RegExp(`^${label}:`) })).toBeVisible()
    }
    await unknowns.getByRole("button", { name: /^Menu and prices:/ }).click()
    await expect(detail.getByTestId("canonical-evidence-sheet")).toContainText("This directory does not include this detail.")
    await detail.getByTestId("canonical-evidence-sheet").getByRole("button", { name: "Close evidence" }).click()
  ...
  test("B-TRUTH-TABLES keeps local-only boundaries and canonical table scope explicit before joining", async ({ page }) => {
    ...
    await expect(truth).toContainText("A joined plan appears in My Korea.")
