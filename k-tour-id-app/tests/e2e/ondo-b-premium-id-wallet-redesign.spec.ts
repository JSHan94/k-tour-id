...
test("contextual benefit pays once, creates a consumer receipt, refunds, and returns to the exact place", async ({ page }) => {
  ...
  await expect(offer).toContainText("Travel balance")
  ...
  await expect(activity).toContainText("Payment 19 OOKRW")
  ...
  await expect(restoredActivity).toContainText("Payment 19 OOKRW")
  ...
  await expect(restoredActivity).toContainText("Refund 19 OOKRW")
  await expect(restoredActivity).toContainText("ONDO-LOCAL-20260825-001")
  ...
  await expect(myReceipts).toContainText("Payment undone 19 OOKRW")
...
test("declining the benefit preserves the 22 OOKRW quote and receipt across reload", async ({ page }) => {
  ...
  const receipt = offer.getByTestId("payment-receipt")
  await expect(receipt).toContainText("This device-only receipt was saved without the ONDO benefit. No purchase occurred.")
  await expect(receipt).not.toContainText("Your ONDO meal benefit is reflected")
  ...
  await expect(page.getByTestId("wallet-activity-receipt")).toContainText("Payment 22 OOKRW")
  await expect(page.getByTestId("wallet-benefit")).toContainText("1 available")
  ...
  await expect(page.getByTestId("wallet-balance")).toContainText("38")
  await expect(restored).toContainText("Payment 22 OOKRW")
  await expect(page.getByTestId("wallet-benefit")).toContainText("1 available")
  await restored.locator("summary").click()
  await restored.getByTestId("wallet-activity-refund").click()
  await expect(page.getByTestId("wallet-balance")).toContainText("60")
  await expect(restored).toContainText("Refund 22 OOKRW")
  await expect(page.getByTestId("wallet-benefit")).toContainText("1 available")
})
