...
async function completeCheckoutGates(page: Page) {
...
test("premium Travel Pass keeps readiness independent and wallet linking consumer-shaped", async ({ page }) => {
  ...
  await sheet.getByRole("button", { name: "Set up local test balance" }).click()
...
test("contextual benefit pays once, creates a consumer receipt, refunds, and returns to the exact place", async ({ page }) => {
  ...
  await expect(offer.getByTestId("payment-confirm")).toContainText("Set up local test balance to continue")
  ...
  // Keep the original duplicate-submission contract on the live JIT-gated
  // checkout: two activations in the same task must still consume one pending
  // action and produce one finalized payment.
  await offer.getByTestId("payment-confirm").evaluate((button: HTMLButtonElement) => {
    button.click()
    button.click()
  })
  await completeCheckoutGates(page)

  const receipt = offer.getByTestId("payment-receipt")
  await expect(offer).toHaveAttribute("data-payment-state", "receipt")
  ...
  await expect(activity).toContainText("Test payment 19 OOKRW Test")
  ...
  await expect(restoredActivity).toContainText("Test payment 19 OOKRW Test")
  ...
  await expect(restoredActivity).toContainText("Test refund 19 OOKRW Test")
  await expect(restoredActivity).toContainText("ONDO-LOCAL-20260825-001")
  ...
  await expect(myReceipts).toContainText("Test refund recorded 19 OOKRW Test")
...
test("declining the benefit preserves the 22 OOKRW quote and receipt across reload", async ({ page }) => {
  ...
  const receipt = offer.getByTestId("payment-receipt")
  await expect(receipt).toContainText("Test payment recorded without the ONDO benefit. No purchase occurred.")
  await expect(receipt).not.toContainText("This local test applied the ONDO meal benefit.")
  ...
  await expect(page.getByTestId("wallet-activity-receipt")).toContainText("Test payment 22 OOKRW Test")
  await expect(page.getByTestId("wallet-benefit")).toContainText("1 available")
  ...
  await expect(page.getByTestId("wallet-balance")).toContainText("38")
  await expect(restored).toContainText("Test payment 22 OOKRW Test")
  await expect(page.getByTestId("wallet-benefit")).toContainText("1 available")
  await restored.locator("summary").click()
  await restored.getByTestId("wallet-activity-refund").click()
  await expect(page.getByTestId("wallet-balance")).toContainText("60")
  await expect(restored).toContainText("Test refund 22 OOKRW Test")
  await expect(page.getByTestId("wallet-benefit")).toContainText("1 available")
})
...
test("QA injection creates recovery without exposing outcome controls", async ({ page }) => {
  ...
  await offer.getByTestId("payment-confirm").click()
  await completeCheckoutGates(page)
  await expect(offer.getByTestId("payment-recovery")).toContainText("Test payment didn’t complete")
  await expect(offer.locator('[data-testid*="outcome"]')).toHaveCount(0)
  await expect(offer.locator("[data-operation-kind]")).toHaveCount(0)
