...
test.describe("SLEEK R5 retry history, resilience truth, and recovery focus", () => {
  ...
  test("R5R-D5-002 offline and map fallback preserve official-directory and source-snapshot truth", async ({ page, context }) => {
    ...
    await expect(offline.getByTestId("ondo-b-location-details")).toContainText("The official directory remains available; map tiles may be unavailable.")
    const resultBar = page.getByTestId("ondo-b-result-bar")
    ...
    await expect(fallback).toContainText("The map could not load. 200 official records remain available in the list.")
