...
test("My Korea preserves a bilingual save while setup edits persona and preferences and localizes independent ID axes", async ({ page }) => {
  ...
  await expect(preferences.getByRole("button", { name: "Local classics", exact: true })).toHaveAttribute("aria-pressed", "true")
  await preferences.getByRole("button", { name: "Cafés and dessert", exact: true }).click()
  await preferences.getByRole("button", { name: "Vegan", exact: true }).click()
  ...
  await identity.getByRole("button", { name: "How these statuses work", exact: true }).click()
  await expect(identity.getByTestId("traveler-id-axis-details")).toContainText("Person does not prove 19+.")
  await expect(identity.getByTestId("traveler-id-axis-details")).toContainText("19+ does not prove identity.")
  ...
  await expect(page.getByTestId("nav-id")).toHaveAttribute("aria-label", "K-Tour ID · ID와 지갑")
  await page.getByTestId("nav-id").click()
  ...
  await identity.getByRole("button", { name: "상태가 작동하는 방식", exact: true }).click()
  await expect(identity.getByTestId("traveler-id-axis-details")).toContainText("본인 확인은 19+를 증명하지 않습니다.")
  await expect(identity.getByTestId("traveler-id-axis-details")).toContainText("19+는 본인을 증명하지 않습니다.")
