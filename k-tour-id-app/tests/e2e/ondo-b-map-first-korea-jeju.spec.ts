  ...
  for (const locale of ["en", "ko", "ja"] as const) {
    test(`${locale.toUpperCase()} keeps one map skeleton across the responsive matrix`, async ({ page }) => {
      for (const viewport of [
        ...
        await expect(cityRoot).toHaveAttribute("data-temperature-visual-grammar", "uniform-editorial-aura-selection-label")
        ...
        await expect(editorialTemperatureKey).toHaveAttribute("data-editorial-temperature-key", "limited")
