...
async function search(page: Page, locale: Locale, query: string, count: number) {
  ...
  const names = page.getByTestId("ondo-b-venue-list").locator("li > button em")
...
test.describe("R3 D5-R3-002 bounded Map food-intent aliases", () => {
  ...
  test("English food intents find only sourced Korean-name matches in Seoul and Busan", async ({ page }) => {
    ...
    await expect(card.locator("strong")).toContainText("Gopija")
    await expect(card.locator("em")).toHaveText("고피자 신촌1호점")
    ...
    await expect(detail).toContainText("고피자 신촌1호점 · Transliterated for navigation")
