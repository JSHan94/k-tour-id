...
test("@core FL-004 checkout success does not create a stamp; a separate unique visit creates the tenth", async ({ page }) => {
  ...
  await expect(page.getByTestId("checkout-overlay")).toContainText("A unique preview visit recorded your tenth stamp. It is not real on-site evidence.")
})
...
test("FL-004 an existing tenth stamp is not attributed to the current checkout", async ({ page }) => {
  ...
  await page.getByTestId("checkout-start").click()
  await page.getByTestId("checkout-confirm").click()
  await expect(page.getByTestId("checkout-overlay")).toContainText("A simulated local-preview visit had already recorded the tenth stamp before this checkout.")
