...
test.describe("ONDO B canonical flow journeys", () => {
  ...
  test(title("FL-001"), async ({ page }) => {
    ...
    await test.step(evidence("FL-001", "RECOVERABLE/LIST"), async () => {
      const map = page.getByTestId("ondo-b-map-entry")
      await expect(map).toHaveAttribute("data-map-state", "ready", { timeout: 12_000 })
      await expect(map).toHaveAttribute("data-map-partial-failure", "recoverable")
  ...
  test(title("FL-015"), async ({ page }) => {
    ...
    await test.step(evidence("FL-015", "ENTRY/DECISION/CANCEL"), async () => {
      ...
      const from = profile.getByRole("button", { name: /From/, pressed: false })
