...
const EXACT_STATES = [
  ...
  "AFTER19-PROMPT", "AFTER19-EXPIRED-REASON", "AFTER19-VENUE-LOCKED", "AFTER19-VENUE-RETURN", "SAVE-FAILURE", "SAVE-RECOVERED",
  ...
  "MY", "SESSION-RESET-CONFIRM", "DISCOVERY-RESET-CONFIRM", "PROFILE", "TRUST-FOUR-AXES", "LABS", "LABS-TRAIT-FAIL", "LABS-BRIDGE-FAIL", "LABS-BRIDGE-SUCCESS",
] as const
...
test("B-EVIDENCE-REGISTRY covers FL-001..018 and every required layout family", () => {
  ...
  expect(B_VISUAL_CASES).toHaveLength(50)
...
test("B-EVIDENCE-FINAL-INTEGRATION makes FL-002 and FL-011 reachable pixel cases", () => {
  ...
  expect(B_VISUAL_CASES.length * B_SLEEK_VIEWPORTS.length).toBe(300)
})

test("B-EVIDENCE-BASELINE-CENSUS owns exactly 300 committed, correctly sized PNGs", () => {
  ...
  expect(mobileFiles).toHaveLength(50)
  expect(desktopFiles).toHaveLength(50)
  expect(responsiveFiles).toHaveLength(200)
  const tracked = execFileSync("git", ["ls-files", "--", "tests/visual/ondo-b-flow-pixels-mobile.spec.ts-snapshots/*.png", "tests/visual/ondo-b-flow-pixels-desktop.spec.ts-snapshots/*.png", "tests/visual/ondo-b-flow-pixels-responsive.spec.ts-snapshots/*.png"], { cwd: process.cwd(), encoding: "utf8" })
    .trim().split("\n").filter(Boolean)
  expect(tracked, "all 300 approved B baselines must be tracked").toHaveLength(300)
  ...
  for (const viewport of B_SLEEK_VIEWPORTS) {
    ...
    expect(files, `${viewport.id} baseline count`).toHaveLength(50)
