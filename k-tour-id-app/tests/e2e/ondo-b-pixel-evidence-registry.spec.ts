...
const EXACT_STATES = [
  ...
  "TABLES-LIST", "TABLE-DETAIL", "TABLE-JOIN-FAIL", "CHAT", "CHAT-IMAGE-FAIL", "FEEDBACK", "REPORT",
...
test("B-EVIDENCE-REGISTRY covers FL-001..018 and every required layout family", () => {
  ...
  expect(B_VISUAL_CASES).toHaveLength(45)
...
test("B-EVIDENCE-FINAL-INTEGRATION makes FL-002 and FL-011 reachable pixel cases", () => {
  ...
  expect(B_VISUAL_CASES.length * B_SLEEK_VIEWPORTS.length).toBe(270)
})

test("B-EVIDENCE-BASELINE-CENSUS owns exactly 270 committed, correctly sized PNGs", () => {
  ...
  expect(mobileFiles).toHaveLength(45)
  expect(desktopFiles).toHaveLength(45)
  expect(responsiveFiles).toHaveLength(180)
  const tracked = execFileSync("git", ["ls-files", "--", "tests/visual/ondo-b-flow-pixels-mobile.spec.ts-snapshots/*.png", "tests/visual/ondo-b-flow-pixels-desktop.spec.ts-snapshots/*.png", "tests/visual/ondo-b-flow-pixels-responsive.spec.ts-snapshots/*.png"], { cwd: process.cwd(), encoding: "utf8" })
    .trim().split("\n").filter(Boolean)
  expect(tracked, "all 270 approved B baselines must be tracked").toHaveLength(270)
  ...
  for (const viewport of B_SLEEK_VIEWPORTS) {
    ...
    expect(files, `${viewport.id} baseline count`).toHaveLength(45)
