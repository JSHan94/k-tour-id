...
test("B-MY-006 the trip memory map visualizes only unique device-local place evidence", () => {
  ...
  expect(my).toContain("...savedEditorial,")
  expect(my).toContain("...recentEditorial,")
  expect(my).toContain("...planned.flatMap(({ editorialPlace }) => editorialPlace ? [editorialPlace] : []),")
  expect(my).toContain("].map((place) => place.id)).size")
  expect(my).toContain("<KoreaMemoryMapB locale={locale} cityCounts={memoryCityCounts} />")
  expect(my).toContain('data-empty-journey={isEmptyJourney ? "true" : "false"}')
  expect(my).toContain('data-testid="my-korea-empty-explore"')
