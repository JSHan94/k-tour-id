...
test("JP-MAP-FIRST-001 Korea overview owns Seoul, Busan, and truthful Jeju anchors", () => {
  ...
  expect(map).toContain('<MapPin size={18} />')
  ...
  expect(map).toContain('data-atlas-x={cityNode.coordinates.x}')
  expect(map).toContain('data-atlas-y={cityNode.coordinates.y}')
  expect(map).toContain('"--atlas-x": `${cityNode.coordinates.x / 3}%`')
  expect(map).toContain('data-atlas-stop={id}')
...
test("JP-MAP-FIRST-001A city selection progressively zooms the existing atlas flow with a reduced-motion fallback", () => {
  const map = source("features/ondo/map/map-entry-b.tsx")
  const css = source("features/ondo/map/map-b.module.css")
  const globals = source("app/globals.css")

  expect(map).toContain("transitionDocument.startViewTransition")
  expect(map).toContain('window.matchMedia("(prefers-reduced-motion: reduce)").matches')
  expect(map).toContain('window.addEventListener("pointerdown", finishBeforeInteraction')
  expect(map).toContain("commitSelection()")
  expect(css).toContain("view-transition-name: ondo-map-surface")
  expect(css).not.toContain(":global(::view-transition")
  expect(globals).toContain("::view-transition-old(ondo-map-surface)")
  expect(globals).toContain("::view-transition-new(ondo-map-surface)")
  expect(globals).toMatch(/::view-transition[\s\S]*pointer-events:\s*none/)
  expect(globals).toContain("ondo-map-surface-out 220ms")
