...
test("FID-P0-013 commerce boundary and persistence rules cannot be weakened", () => {
  ...
  expect(commerce).toContain("No wallet, merchant, stablecoin network or payment provider is currently connected")
  expect(commerce).toContain("현재 외부 지갑·가맹점·스테이블코인 네트워크·결제 제공자는 연결되어 있지 않습니다")
  expect(commerce).toContain("device ledger uses OOKRW internally")
  expect(commerce).toContain("source may be USDC or USDT")
  expect(commerce).toContain("기기 내 원장은 OOKRW를 내부 단위로 사용")
  expect(commerce).toContain("출처는 USDC 또는 USDT")
  ...
  expect(commerce).toContain("외부 주문이나 실제 돈 이동은 없어요")
  expect(commerce).toContain("外部注文や実際のお金の移動はありません")
  ...
  expect(commerce).toContain("restoreBActionGateSession(window.sessionStorage, new Date(), actionGateSessionOptions())")
  expect(commerce).toContain("consumePendingBActionAtMutation(window.sessionStorage")
  // A funding operation may recover after a sheet closes. That receipt is
  // session-only and never restores a balance by itself or persists identity.
  const fundingStart = commerce.indexOf("function FundingSourceSheet(")
  const fundingEnd = commerce.indexOf("function CanonicalCommerceOfferB(", fundingStart)
  expect(fundingStart).toBeGreaterThan(0)
  expect(fundingEnd).toBeGreaterThan(fundingStart)
  const funding = commerce.slice(fundingStart, fundingEnd)
  const otherCommerce = commerce.slice(0, fundingStart) + commerce.slice(fundingEnd)
  expect(otherCommerce).not.toMatch(/sessionStorage\.(?:setItem|removeItem)/)
  const mutations = [...funding.matchAll(/sessionStorage\.(?:setItem|removeItem)\(([^,\)]+)/g)]
  expect(mutations.length).toBeGreaterThan(0)
  for (const mutation of mutations) expect(mutation[1]).toBe("FUNDING_RAIL_SESSION_KEY_B")
  expect(funding).toContain("readFundingRailB(window.sessionStorage.getItem(FUNDING_RAIL_SESSION_KEY_B))")
  expect(funding).toContain("state.commerceSession.fundingCredits.some")
  expect(funding).toContain("if (!credited)")
  expect(commerce).not.toMatch(/dateOfBirth|passport|documentNumber/i)
