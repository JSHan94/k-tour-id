...
export const SUMSUB_PASSPORT_DISCLOSURE = {
  en: { scope: "Sandbox · identity check only", purpose: "Test the passport verification flow", evidence: "Passport and selfie / liveness", provider: "Sumsub Sandbox", retention: "Use test materials only. Documents and selfies may be sent to Sumsub Sandbox. ONDO does not store the original images. Closing this screen does not delete data already sent to the provider.", boundary: "This is a Sandbox identity check, not a verified K-Tour ID. No pass is issued and age, residency, payments and benefits stay unchanged.", returnStep: "Return" },
  ko: { scope: "Sandbox · 신원 확인 테스트", purpose: "여권 신원 확인 흐름 테스트", evidence: "여권과 셀피·실재성 확인", provider: "Sumsub Sandbox", retention: "테스트 자료만 사용하세요. 문서와 셀피가 Sumsub Sandbox에 전송될 수 있어요. ONDO는 원본 이미지를 저장하지 않아요. 화면을 닫아도 이미 제공자에게 전송된 자료는 삭제되지 않아요.", boundary: "Sandbox 신원 확인 테스트이며, 인증된 K-Tour ID가 아니에요. 패스는 발급되지 않고 나이·체류 자격·결제·혜택 권한은 바뀌지 않아요.", returnStep: "돌아가기" },
  ja: { scope: "Sandbox・本人確認テスト", purpose: "パスポート本人確認フローのテスト", evidence: "パスポートとセルフィー・実在性確認", provider: "Sumsub Sandbox", retention: "テスト資料のみ使用してください。書類やセルフィーがSumsub Sandboxに送信される場合があります。ONDOは元画像を保存しません。画面を閉じても送信済みデータは事業者から削除されません。", boundary: "Sandboxの本人確認テストであり、認証済みK-Tour IDではありません。パスは発行されず、年齢・滞在資格・決済・特典の権限は変わりません。", returnStep: "戻る" },
} as const
