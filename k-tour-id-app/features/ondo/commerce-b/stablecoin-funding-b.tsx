...
export function StablecoinFundingB({ operation, locale, closing, creditComplete, balance, returnLabel, onAction, onAmount, onCreditRetry, onUseBalance, onMethods, onClose }: Props) {
  ...
  return <div className={styles.root} data-testid="stablecoin-funding" data-stage={stable.stage} data-source-status={stable.sourceStatus} data-destination-status={stable.destinationStatus}>
    ...
    <ol className={styles.steps} aria-label={w("Stablecoin funding steps", "스테이블코인 충전 단계", "ステーブルコインのチャージ手順")}>
    ...
    {phase === "quoted" || phase === "authorize" ? <>
      ...
      </> : <p className={styles.notice}><ShieldCheck size={17} aria-hidden="true" />{w("Approve this amount only. Your signer is not your DID or payment verification.", "이 금액만 승인합니다. 지갑 서명은 DID·결제 자격 확인과 별개예요.", "この金額だけを承認します。署名はDIDや決済確認とは別です。")}</p>}
      ...
      <ol className={styles.journey} aria-label={w("Transfer status", "전송 현황", "送信状況")}>
        ...
        <li data-complete={destinationDone}><span>{destinationDone ? <Check size={15} aria-hidden="true" /> : 2}</span><div><strong>{w("OmniOne route", "OmniOne 경로", "OmniOne経路")}</strong><small>{destinationDone ? w("Destination confirmed · sample", "도착 확인 · 샘플", "到着確認・サンプル") : w("Interoperability hypothesis", "상호운용 가설 시연", "相互運用の仮説デモ")}</small></div></li>
