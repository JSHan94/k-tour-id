...
export function WalletConnectionPreviewB({ locale, method, onApprove, onReturn }: {
  ...
  return <section className={styles.connectionPreview} data-testid="stablecoin-connection-preview" data-status={status} aria-labelledby="wallet-connection-heading">
    ...
    <dl className={styles.facts}><div><dt>{w("Requested by", "요청 서비스", "リクエスト元")}</dt><dd>ONDO</dd></div><div><dt>{w("Permission", "요청 권한", "権限")}</dt><dd>{w("Connect sample wallet only", "샘플 지갑 연결만", "サンプル接続のみ")}</dd></div><div><dt>{w("Network", "네트워크", "ネットワーク")}</dt><dd>Sui Testnet · {w("sample", "샘플", "サンプル")}</dd></div></dl>
    {status === "waiting" ? <>
