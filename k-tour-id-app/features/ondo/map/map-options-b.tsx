...
import { BookOpen, Check, ChevronDown, ChevronRight, FlaskConical, Languages, Layers2, LocateFixed, MoonStar, ShieldCheck, SlidersHorizontal } from "lucide-react"
...
const COPY = {
  en: {
    ...
    language: "Language",
    after19: "After 19 picks", after19Body: "Turn off After 19 on the map to browse every category.",
  },
  ko: {
    ...
    language: "언어",
    after19: "After 19 추천", after19Body: "다른 분류를 보려면 지도에서 After 19를 꺼 주세요.",
  },
  ja: {
    ...
    language: "言語",
    after19: "After 19 のおすすめ", after19Body: "他のカテゴリを見るには、地図で After 19 をオフにしてください。",
  },
} as const
...
export function MapOptionsB({
  ...
  locateLabel, onLocate, onPreferences, onStories, languageLabel, onLanguage,
  onDemo, onClose,
}: MapOptionsBProps) {
  ...
  return <SheetB
    ...
    variant="decision"
    onClose={onClose}
    shouldRestoreFocus={() => !guideOwnsFocus.current}
    ...
    <div className={styles.body} data-testid="ondo-b-map-options" data-category-locked={String(categoryLocked)}>
      <section className={styles.section} aria-labelledby="ondo-map-options-categories">
        ...
        {categoryLocked ? <div className={styles.nightNotice} data-testid="ondo-b-map-options-after19">
          <MoonStar size={20} aria-hidden="true" /><div><strong>{copy.after19}</strong><p>{copy.after19Body}</p></div>
