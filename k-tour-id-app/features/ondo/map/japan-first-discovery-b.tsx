...
const COPY = {
  en: {
    ...
    content: "Top stories",
    more: "6 more stories being checked",
    jeju: "Jeju · Growing",
...
export function JapanFirstDiscoveryB({ locale }: { locale: OndoBLocale }) {
  const copy = COPY[locale]
  const featured = JAPAN_FIRST_FEATURED_CONTENT_IDS.map((id) => JAPAN_FIRST_LAUNCH_CONTENT.find((item) => item.id === id)!)
  const remaining = JAPAN_FIRST_LAUNCH_CONTENT.filter((item) => !JAPAN_FIRST_FEATURED_CONTENT_IDS.includes(item.id as typeof JAPAN_FIRST_FEATURED_CONTENT_IDS[number]))
  const jejuSources = [...new Map(JEJU_EDITORIAL_SEEDS.map((item) => [item.sourceUrl, item])).values()]
  return (
    <details className={styles.root} data-testid="ondo-b-japan-first-discovery">
      <summary>
        <span className={styles.mark}><Sparkles aria-hidden="true" size={20} /></span>
        <span>
      ...
      <div className={styles.panel}>
        ...
        <details className={styles.moreStories} data-testid="ondo-b-japan-more-stories">
          <summary>{copy.more}<ChevronRight aria-hidden="true" size={16} /></summary>
          <div>
