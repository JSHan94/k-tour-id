  ...
  return (
    <Sheet label={locale === "ko" ? "결제 시뮬레이션" : "Checkout simulation"} onClose={returnToVenue} size="full">
      <div className={styles.body} data-payment-state={checkout.payment} data-payment-kyc={state.paymentKyc} data-stamp-count={state.stamps} data-venue-id={venueId} data-testid="checkout-overlay">
        ...
        <InlineNotice tone="neutral"><ShieldCheck size={18} /><span>{locale === "ko" ? "실제 결제나 자산 이동이 발생하지 않습니다. 결제용 KYC는 본인 확인·19+와 별도입니다." : "No real payment or asset movement occurs. Payment KYC is separate from identity and 19+ checks."}</span></InlineNotice>
        ...
        {checkout.payment === "PAY-SIMULATED-SUCCESS" ? (
          <div className={styles.visitProof}>
            ...
            {state.stamps === 10 && visitState !== "accepted" ? <InlineNotice tone="neutral"><Check size={18} /><span>{locale === "ko" ? "시뮬레이션된 로컬 미리보기 방문으로 이 결제 전에 열 번째 스탬프가 이미 기록되었습니다." : "A simulated local-preview visit had already recorded the tenth stamp before this checkout."}</span></InlineNotice> : null}
