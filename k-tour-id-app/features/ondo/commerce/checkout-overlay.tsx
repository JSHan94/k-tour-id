...
export function CheckoutOverlay({ venueId }: { venueId: string }) {
  ...
  return (
    <Sheet label={locale === "ko" ? "결제 시뮬레이션" : "Checkout simulation"} onClose={() => actions.setSurface({ kind: "venue", venueId })} size="full">
      <div className={styles.body} data-payment-state={checkout.payment} data-payment-kyc={state.paymentKyc} data-stamp-count={state.stamps}>
        ...
        {checkout.payment === "PAY-IDLE" ? <button type="button" className={styles.primary} onClick={start}>{ready ? locale === "ko" ? "시뮬레이션 확인" : "Confirm simulation" : locale === "ko" ? "결제용 KYC부터 계속" : "Continue with Payment KYC"}</button> : null}
        {checkout.payment === "PAY-CONFIRMING" ? <><button type="button" className={styles.primary} onClick={confirm}>{locale === "ko" ? "이 내용으로 시뮬레이션" : "Run this simulation"}</button><button type="button" className={styles.secondary} onClick={cancel}>{locale === "ko" ? "취소" : "Cancel"}</button></> : null}
        ...
        {checkout.payment === "PAY-SIMULATED-SUCCESS" ? (
          <div className={styles.visitProof}>
            ...
            {state.stamps < 10 ? <button type="button" className={styles.secondary} onClick={verifyVisit} disabled={visitState === "checking"}>{visitState === "checking" ? locale === "ko" ? "현장 방문 확인 중" : "Checking on-site visit" : locale === "ko" ? "별도 현장 방문 확인" : "Check separate on-site visit"}</button> : null}
            {visitState === "accepted" || state.stamps === 10 ? <InlineNotice tone="success"><Check size={18} /><span>{locale === "ko" ? "별도의 중복되지 않은 방문 증거로 열 번째 스탬프를 남겼어요." : "A separate unique visit proof recorded your tenth stamp."}</span></InlineNotice> : null}
