...
This branch tests product structure, wording, interaction hierarchy and visual direction. The B variant now renders an interactive OpenStreetMap raster basemap with Leaflet; K-Tour places are positioned from their mock coordinates. Search results, route estimates, quotes and partner availability remain simulated. The public OpenStreetMap tile service is used only for this low-traffic prototype and has no SLA, so the production tile/search/routing provider, OpenDID/OpenCX contracts and Sui implementation remain developer-owned decisions governed by the development specification.
