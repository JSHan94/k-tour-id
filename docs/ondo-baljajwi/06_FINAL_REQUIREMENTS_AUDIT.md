...
이 문서는 사용자가 처음 제시한 다섯 가지 실행 질문과 열아홉 가지 제품·기술 아이디어가 최종 제품에 어떤 깊이로 반영됐는지 판정하는 최종 source of truth다. 이전 [`ondo-execution`](../ondo-execution/00_EXECUTION_INDEX.md) 문서는 A/v2의 역사 기록이며, 현재 B 후보 판정에는 이 문서, [Final As-built](./07_AS_BUILT.md), [B Evidence Manifest](./04_EVIDENCE_MANIFEST.md)를 우선한다.
...
| 만든 것의 명세가 빠짐없이 남는가? | `예` | 19/19 REQ, FL-001~018, 121 ACTUAL checkpoint, 5 사유 있는 N/A, 44상태×2 viewport, KO/EN 14 surface, 외부 연동 등급을 추적한다. 최종 구현·테스트·리뷰·배포는 Evidence/As-built에 기록한다. |
...
| 많은 병렬 agent/token을 써도 충돌 없이 실행 가능한가? | `예, 현재 보강된 실행팩 기준` | Root/Map/Identity/Connect 경로 소유, frozen contracts, CCR, shared SHA 동기화, durable evidence, 두 번의 동일-SHA clean round가 있다. 토큰 소진 자체를 품질로 보지 않고 Gate 실패·미해결 issue만 루프 조건으로 쓴다. |
...
| 18 | 웹앱 우선 | responsive Next.js web route `/ondo-b` | `ACTUAL` | 390×844·1440×1000, KO/EN, keyboard, fallback | native iOS/Android 앱 |
...
현재 프론트엔드 데모를 완성·검수·별도 B preview로 배포하는 데 추가 결정은 필요 없다. 다음 항목을 `실연동`으로 승격할 때만 새로운 사업·보안·법무 결정이 필요하다.
...
- 44 layout states를 mobile/desktop 각각 검증하고 clipping, nav/CTA overlap, 12px 미만 metadata, 44px 미만 control, serious/critical Axe issue가 없어야 한다.
- 다섯 독립 역할이 같은 tuple에서 actionable S0/S1/S2를 0으로 판정한 clean round가 두 번 연속이어야 한다.
- 제품·harness가 바뀌면 clean streak는 0으로 돌아간다. 문서와 evidence-only commit은 tuple을 바꾸지 않는다.
