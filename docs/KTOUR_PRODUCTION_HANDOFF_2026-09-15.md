...
기록일: 2026-09-15. 상태: **Ready — 운영 배포·지정 여정 검수 완료**. **최신 앱 코드 기준은 `505e4757e202a96492044f85e0a5b2bb2e75ff67`**이다. 아래 역사적 브랜드 릴리스의 `4cb1964`와 구분한다.
...
현재 순수 목업의 매장 액션, 충전/주문 복귀, 공통 환불, 주문별 내역, 사진 콘텐츠를 정리했다. 실제 CX/OpenDID/OmniOne Chain/Sui 실연동 범위는 그대로 개발 대상이다. 비금전 Sui 전용 목업 추가 여부는 UX-08 결정 대기이며 금융 목업을 그 완료 증거로 쓰지 않는다. Sumsub Sandbox는 아래 별도 경로를 유지한다.

검수 기준 SHA와 이후 결과 기록·테스트 하네스만 바꾼 커밋을 구분한다. 후속 커밋의 앱 배포 입력이 `505e475`와 동일한지 확인하고 main/Harvey에 함께 반영한다. 배포별 실제 실행 수와 실패 재검사 이력은 UX 릴리스에 기록한다.
