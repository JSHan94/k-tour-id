# ONDO 溫圖 — K-Tour ID × Sui 개발 브리프

> **역사적 별도 프로그램 브리프.** Sui는 현재도 사용자 지정 프로젝트 범위지만 아래 프로그램의 참가 상태·최신 조건은 확인된 것이 아니다. 이를 DID 대회의 공식 필수 조건이나 현재 목업의 미완료 판정으로 사용하지 않는다. 현행 signer/스테이블코인/badge·cross-chain 경계는 [DEPLOYMENT_SPEC G09-S·G12·§8](./DEPLOYMENT_SPEC.md), 구현 배정은 [개발자 시작 문서](./DEVELOPER_START_HERE.md)를 따른다.

상태: `필수 개발 범위 · 상세 설계는 개발자 확정`

이 문서는 ONDO 溫圖의 K-Tour ID 기능 범위에 Sui 에코시스템 빌더 지원 프로그램의 기술 요건을 포함하기 위한 제품 브리프다. ONDO 브랜드 변경은 이 범위를 삭제하거나 축소하지 않는다. 제품팀은 결과와 검수 기준을 고정하고, 개발자는 실제 구현 사양을 ADR로 정의한다.
