...
> **Historical A/v2 execution record.** 최종 ONDO B 제품과 19개 요구 판정의 현재 정본은 [ONDO B Final Requirements Audit](../ondo-baljajwi/06_FINAL_REQUIREMENTS_AUDIT.md)이다. 아래 SHA·Leaflet·초기 fixture 범위는 당시 A 후보의 기록이며 현재 B 구현을 설명하지 않는다.
...
| `FL-002` Age proof → exact After19 venue | 005, 012 | `E2E-CORE-03`, `E2E-CORE-04` | age-only returnTo→같은 venueId 확장 상세→After19 ON | 취소·거절·만료에도 같은 장소 공식 정보와 일반 ONDO 유지 |
