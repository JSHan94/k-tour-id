...
상태: `v2.1 · 개발 핸드오프 기준 문서 · 2026-08-11`

대상: 제품, 프론트엔드, 백엔드, 모바일, DID/VC, 블록체인, AI, 결제, 보안, QA, DevOps

이 문서는 검증된 클릭 목업을 실제 제품으로 구현하기 위한 기능·비즈니스·연동·품질 계약이다. 화면은 사용성 샘플이며, 이 문서의 상태·정책·데이터·증거 요구를 모두 충족해야 개발 완료로 판정한다.
...
- [선택적 외부 체인 상호운용 메모](./EXTERNAL_CHAIN_INTEROPERABILITY.md)
...
6. OmniOne Chain에 실제 Solidity 스마트컨트랙트를 배포하고 최소 한 개의 제품 불변식과 비식별 실행 증거를 검증한다.
7. 개인정보 없는 canonical event와 공급자·OmniOne Chain receipt가 Evidence에서 검증된다.
...
- 체인이나 외부 영구 저장소에 PII·VC 원문을 저장하는 것
...
| AI 도우미 | 제한된 도구 권한 | 추천·계획·사용자 승인 후 행동 요청·실행 근거 기록 |
...
- 동의 거절, 외부 앱 미설치, QR 만료, 사용자 취소, provider 비활성, callback 위조/중복, 토큰 만료/불일치, 수동검토, holder 저장 실패, VC 발급 실패, OmniOne Chain 기록 지연.
...
단기 해외 방문자의 기본 계정 생성은 한국 휴대전화 번호를 요구하지 않는다. 이메일/OAuth/passkey 등 실제 채택 방식은 auth ADR에서 정하되, 신원확인용 여권 데이터가 곧 계정 비밀번호나 복구 수단이 되어서는 안 된다.
...
- OAuth/passkey 또는 DID holder key 복구 정책은 별도 ADR에 따른다.
...
- 외부 지갑 주소를 지원하더라도 K-Tour DID나 정부 신원과 동일시하지 않고 연결 정보를 최소화한다.
...
- 여행 여정 기반 digital souvenir/NFT는 P2다. 사용 권리·utility·transfer·wallet recovery·삭제/민원·수수료가 정의될 때만 발행하며, 단순 발행을 블록체인 핵심 통합으로 완료 처리하지 않는다.
...
### F-090 AI Benefit Router와 승인 실행
...
- AI가 구매·예약·혜택 사용을 요청할 때는 승인 내용, policy version, idempotency key와 실행 receipt를 남긴다. 계약으로 강제할 필요가 있는 1회성 혜택·재실행 방지는 OmniOne Chain 스마트컨트랙트가 맡을 수 있다.
...
- OmniOne Chain contract address, transaction hash와 receipt
- 외부 체인을 선택적으로 사용한 경우 별도 adapter receipt
...
flowchart LR
  ...
  B --> AI[AI Benefit Router]
  AI --> C
  B --> O[Transactional Outbox]
  O --> OMNI[OmniOne Chain Smart Contracts]
  DID --> E[Evidence Service]
  PAY --> E
  EXT --> E
  OMNI --> E
  O -. approved interoperability only .-> EXTCHAIN[Optional External Chain Adapter]
  EXTCHAIN --> E
```
...
- 공식 제공 환경에 Solidity source 또는 ABI + Bytecode를 등록하고 실제 contract를 배포한다.
- EOA 서명 트랜잭션과 제공 RPC/API를 통해 state-changing call과 read-only call을 재현한다.
- 최소 한 개의 제품 불변식을 계약이 직접 거절하도록 한다. 기본 후보는 바우처 중복 사용, 캠페인 정책 버전 불일치, 만료된 승인 재실행이다.
- `operationId`의 직접 노출 대신 회전 가능한 비식별 reference와 canonical payload hash를 event에 기록한다.
- contract receipt는 Evidence에서 검증하되 주문·결제·환불·정산의 source of truth로 사용하지 않는다.
- contract 지연·실패는 outbox, retry, dead-letter, reconciliation으로 처리한다.

정확한 network/chain ID, RPC/API, explorer, Solidity/compiler 호환 버전, contract state·role·upgrade 방식, EOA/API key custody는 ADR-OMNI-001에서 개발자가 정의하고 멘토에게 확인받는다.

### 8.4 선택적 외부 체인 상호운용

외부 파트너가 특정 체인의 영수증이나 attestation을 요구할 때만 adapter로 추가한다. Sui 같은 타 체인은 이 선택지의 한 예일 뿐이며 K-Tour ID의 P0·핵심 발표·기본 배포 요건이 아니다. 동일 사건을 여러 체인에 복제하려면 제거 불가능한 사업상 이유, source of truth, 비용, 개인정보, 불일치 대사 방식을 별도 ADR로 승인받는다.
...
- chain/external immutable storage: no restricted/confidential data, explorer scan, pseudonym rotation
...
| 관측성 | trace ID, domain metrics, provider latency/error, ledger mismatch, OmniOne Chain confirmation lag |
...
- [ ] AI propose → user approve → policy recheck → service action → receipt
...
- [ ] OmniOne Chain duplicate redemption, unauthorized caller, expired policy, RPC/receipt failure
- [ ] OmniOne Chain submission pending/dead-letter and ledger reconciliation

### 13.3 privacy/security

- [ ] repo/log/DB/chain/외부 저장소에서 여권번호·얼굴·VC 원문·private key·OAuth JWT가 검출되지 않음
...
| G5 OmniOne Chain | Solidity contract + 제품 불변식 + 비식별 receipt | 제공 환경 transaction evidence와 negative tests |
...
| fake `did:omn`, tx hash | 실제 OpenDID/OmniOne Chain receipt 또는 명시적 simulation |
...
- ledger·provider·OpenDID·OmniOne Chain receipt가 operation ID로 대사된다.
- 자동화·보안·개인정보·접근성 검수의 Critical/High issue가 0개다.
- 공개 README, deployment/runbook과 evidence bundle이 준비됐다.
...
- 2026 블록체인 & AI 해커톤 공식 가이드북 — OmniOne CX 필수과제, OpenDID·OmniOne Chain 선택과제와 Chain 스마트컨트랙트 배포·호출 절차
...
- 실제 파트너 요구가 승인된 경우 해당 외부 체인의 공식 문서
