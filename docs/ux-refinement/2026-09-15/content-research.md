...
최신 상태(2026-09-16): **신설오름1곳·몸국 사진1장의 콘텐츠 전용 추가를 승인받아 소스에 반영, 독립 실제 UI/배포 검수 대기**. 새 catalog는 연구26곳·실제사진3장, 기존 서비스 capability27곳·canonical400·curated pulse80은 유지한다. 이전 완료한 어니언·학림2장/학림1곳의 배포 증거와 합산하지 않는다. 최종 파일·해시·허용 범위는 [media-manifest.json](media-manifest.json), 후속 연구/승인은 [9/16 시각·콘텐츠 검토](review-20260916-visual-content.md), 실제 검수·배포는 [릴리스](RELEASE.md)를 따른다.
...
이 절은2026-09-15 당시 판단이다. 신설오름의 두 번째 좌표/새 콘텐츠 보류는 아래9/16 후속 결과로 해소됐으며, 이전 사진·좌표 확인일은 덮어쓰지 않는다.
...
## 8. 2026-09-16 신설오름 구현 — 신규 검수 대기

- 기존 후보 ID를 보존해 `research-jeju-sinseoloreum`을 제주 연구 JSON에1곳 추가했다. 공식 VisitJeju 좌표33.50546,126.54163을 사용하고 KTO 공개 GET의33.5053848808039,126.541578109031/동일 주소로 교차 확인했다. 출처 API/URL·확인 방법은9/16 검토 문서와 후보 JSON에 기록한다. 기존25곳의 checkedAt/사진/소스는 변경하지 않았다.
- Commons 공석배 CC BY4.0 몸국 원본(2019-07-23 촬영)을 그대로 local JPEG로 전달했다.961×961/229,740bytes, SHA256 `1550a36f18e307d644c5856573de445e0ae62530137cb1d4f2e945c983885666`. 원본에는 식별 가능한 인물이 없음을 직접 확인했다. EN/KO/JA alt·저자/원본/라이선스·촬영일/화면 크롭 credit을 연결했다. 알려진 촬영일을 `publishedAt`나 현재 메뉴 증거로 쓰지 않았다.
- 구현한 것은 로컬 사진 경로·기존4:3 카드/1:1 상세·배포 allowlist 한 항목뿐이며, no-photo 분류 caption은9→11px로 조정했다. 서비스 registry·결제/예약/혜택·실시간 activity·방문 증명은 추가하지 않았다. Momos는 원본 정상 취득 조건이 남아 이번 배치에서 반영하지 않는다.
- 실제 crop·이미지실패·크레딧/같은 장소·unsupported footer와 신규 build/배포 검수는 별도 담당자가 수행하기 전까지 PASS로 기록하지 않는다. 작성자가 자기 구현의 최종 시각 수락을 대신하지 않는다.
