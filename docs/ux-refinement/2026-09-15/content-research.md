...
상태: Phase A 초기 조사와 후속 콘텐츠 게이트. 기준 앱 `b9cdc42`, 조사일 `2026-09-15`. 초기 조사 당시 앱 반영/자산 생성은 0건이었다. 후속 게이트에서는 **기존 어니언 사진 1건을 승인**했고 root가 로컬 파일을 추가했다. **새 장소 삽입 승인 0건**, 학림 1곳은 다음 콘텐츠 승인용 초안이다. 실제 앱 크롭/오류/크레딧 QA와 배포는 별도다. [실행 계획](../../UX_REFINEMENT_EXECUTION_PLAN_2026-09-15.md)의 권리·ID 보존·PRD 게이트를 적용하며, 이 문서만으로 UX 전체 합의나 배포 PASS를 선언하지 않는다.
...
[media-manifest.json](media-manifest.json)은 현재 8건이다: 기존 장소 2건, 새 후보 2곳의 사진 3건, 지점/권리 충돌 예시 3건. 이 중 `integrationAllowed: true`는 어니언 1건뿐이다. 학림 실내 사진은 인물이 주목적으로 보여 보류했으며, 간판 사진을 대안으로 추가했다. 파일별 권리·날짜 의미·지점·크롭·제안 경로·금지 용도를 분리했다.
