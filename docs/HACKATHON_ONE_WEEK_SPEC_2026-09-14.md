...
작성일: 2026-09-14 · 개발 인계안 v2 · 대상: 구현 개발자, 기술 담당자, 시연 담당자

**2026-09-14 제품 책임자 요청으로 OmniOne CX + OpenDID + OmniOne Chain 모두 이번 팀의 필수 구현 범위다.** 인원·공급자 환경·작업 가능 시간은 아직 확인 전이다. [해커톤 연동 개발 요약](./HARVEY_HACKATHON_HANDOFF_2026-09-14.md)에서 범위·대표 플로우·작업·리소스를 먼저 확인하고, 이 문서의 기술별 상세·API·데이터 계약·검사를 구현 기준으로 사용한다. 아래 API·타입·설정은 새 구현 제안이며 이미 연결된 서버나 공급자 공식 규격이 아니다.

## 0. 먼저 읽을 한 페이지

### 이번 주의 결과

**지도 속 장소 1곳 → 비금전 체험 혜택 1개 → 실제 모바일 신분증 확인 → K-Pass 발급·보관·제시 → 서버 자격 판정 → 혜택 1회 사용 → 실제 체인 기록 → 같은 장소 복귀.**

- 국내 Mobile ID 경로 1개, 실제 holder 1종, 시연자 1명 이상, 장소 1곳, 캠페인 1개, 체인 계약 1개만 연결한다.
- 신원 확인은 실제 공급자 결과, VC/VP는 실제 OpenDID 결과, 업무 기록은 실제 OmniOne Chain 영수증으로 검증한다. 서비스 혜택은 **금전·실물 제공 의무 없는 해커톤 체험 권한**이다.
...
초안은 숙련 개발자 **2–3명**, 첫날 테스트 환경·실기기 확보, 기존 OpenDID 서버/실제 holder 재사용을 가정했다. 현재 Harvey의 연동 담당과 woogieboogie 측 FE·QA 지원을 제안하며, 실제 가용 인원·시간을 확인하기 전 일정 달성을 보장하지 않는다. 신규 holder 앱·신뢰환경을 처음부터 만드는 일정은 포함하지 않는다. 9/15 정오까지 환경 준비 상태를 판정하고 당일 종료까지 세 기술 smoke를 목표로 한다.
...
| S: 후속 선택 | Sui 실제 테스트 거래 1건 | **이번 인계 구현에서 제외**. 별도 승인 없이 추가하지 않음 |
| 제외 | 실제 원화/스테이블코인 충전·결제·환불, 상점 예약 공급자, Passport/Residence, 교통·배달, 새 지도·AI·채팅 시스템 | 기존 목업 보존, 후속 주차로 이관 |

M0만 연결하면 이번 팀 필수 구현 완료가 아니다. 환경 지연으로 M1을 자동 제외하지 않는다. 지원·추가 인력·환경 해결을 먼저 요청하며, 범위 변경은 제품 책임자의 새 명시적 승인 사항이다. 1인이거나 환경이 없으면 일정 전제를 재협의한다.
...
사업 제안서의 신원 정규화 → K-Pass → 서비스 권한 → 업무 기록을 한 개의 체험 혜택으로 축소 구현한다. **외국인 온보딩·체류권·Payment KYC·지역 주민 혜택을 Mobile ID 한 번으로 입증했다고 주장하지 않는다.** 원화 스테이블코인·Sui는 전체 제품 요구에 남지만 공식 필수 기술로 묶지 않는다. 사업 PDF의 개인정보·연락처는 개발 자료/영상에 복사하지 않는다.
...
역할 제안: A=Harvey(Identity·서비스 서버·Chain), B=woogieboogie 측(FE·QA), C=필요 시 추가 DID·Chain 지원자(미정). 2명이어도 서비스 서버·Chain을 FE 담당에게 자동 이전하지 않는다. 실제 가용 시간과 지원 인원을 확인한 후 확정하며, 제품 책임자는 계정·주관사 문의·시연자를 지원한다.
...
**Gate:** 9/15 정오까지 세 기술의 환경 준비 상태·blocker를 공유하고, 당일 종료까지 실제 smoke를 목표로 한다. 어느 하나라도 미확보이면 원인·지원 담당·해결 기한·추가 인력 필요를 즉시 팀에 공유한다. **OpenDID/Chain을 자동 제외하거나 CX-only를 이번 팀 완료로 처리하지 않는다.** 현재 세 기술 필수 범위를 유지하며 지원·일정·환경 대안을 협의한다. 범위 변경은 제품 책임자의 별도 새 승인 사항이다. 샘플을 실제 연결로 바꾸어 말하지 않으며 정부 SDK 전환도 별도 승인·환경 확인 없이는 즉석 우회책으로 취급하지 않는다.
...
장소 상세 → 체험 혜택 확인 → 이용 조건·정보 제공 동의
 ...
 → 서버 자격 판정 → “혜택 사용” 별도 확인 → 서버 1회 사용 기록
 → 같은 장소로 복귀
...
| 동의 거절/취소 | 같은 장소로 돌아가기 | 해당 요청 종료, 발급/사용을 임의 생성하지 않음 |
...
- 사용 버튼을 눌렀을 때 서버가 만료·철회/status freshness·캠페인·미사용을 다시 확인한다. D1 정책 기본값은 proof/decision TTL 5분, status freshness 최대 60초이며 제공 SDK 제약과 시연 정책에 맞게 고정 기록한다. 이는 법적 연령/금융 정책이 아니다.
- 서버 권한 확인 → 해당 `decisionRef` 1회 소비 → redemption 1건 + outbox 1건을 하나의 transaction으로 기록한다. 이미 VP 단계에서 소비한 challenge nonce를 다시 소비하지 않는다. 중간 장애 시 rollback. 동일 idempotency key/동일 body는 같은 결과, 다른 body는 conflict.
...
| `POST /demo-entitlements/{id}/redeem` | presentationId·decisionRef·consentDigest·idempotency key → 서버 재검증 후 redemption operation. client subject/allow를 신뢰하지 않음 |
...
```ts
// 앱/BFF 정규화 제안. SDK raw response 또는 기존 mock enum과 동일하지 않음.
...
| D2 (9/15) | 정오 환경 Gate, 당일 CX/OpenDID/Chain 각각 실제 smoke. API/DTO·장소 action·복귀 계약 고정 | CX 서버 검증·VC 보관/VP 검증·Chain receipt. 미확보 항목은 즉시 팀 결정 요청 |
| D3 (9/16) | VP verifier·status·정책 binding, 혜택 DB unique·outbox, Chain 계약/worker | 실제 proof로 allow, 잘못된 요청 deny, 1회 사용+체인 receipt |
| D4 (9/17) | 장소에서 끝까지 통합, pending/취소/unknown·재조회 | 공개 staging에서 실제 여정 1회 완주. 미완주면 추가 기능 전부 중단 |
| D5 (9/18) | 실기기 앱 왕복·cold return·공격/중복·만료·철회 검수 후 기능 동결 | 아래 A01–A12 결과와 증거. 신규 기능 추가 없음 |
...
|---|---|---|---|
| HK-00 | 책임자+A | 9/14–15 환경·정책·시연 기기/계정 확보, 세 필수 기술의 환경 Gate 결정 | BE-01 일부·기술 ADR |
...
| HK-05 | 전원 | A01–A12·모바일 회귀·녹화·제출 패키지 | 영향 범위 QA |
...
| A02 | CX→VC/holder→VP→별도 사용 확인 | 같은 subject·venue·campaign으로 사용 1건, 실제 공급자/체인 증거 연결 |
...
| A09 | 실제 외부 앱을 거친 cold return | 로그인/operation 재확인 후 같은 장소·검색/목록·카메라 복원; 취소면 미사용 |
...
## 9. Sui와 나머지 기능은 어떻게 둘까?

이번 인계에서는 Sui를 구현 범위에서 제외한다. 전체 제품 요구에서 삭제한 것은 아니다. 아래 내용은 별도 승인·담당·일정이 생길 때 사용할 후속 선택안이며, 이번 주 세 필수 기술과 A01–A12 검수를 지연시키지 않는다.

선택 시에도 **기존 signer 한 종류 + 허용된 Sui 테스트 거래 하나 + 실제 effects 조회·거절/재조회**로 한정한다. 테스트 자산/가스/계정이 이미 준비되어 있어야 한다. 기존 Labs의 분리된 증거 화면을 사용하고 방문 증거 없이 방문 badge를 발급하지 않는다. funding·destination credit·실제 KRW 잔액을 바꾸지 않는다.

Sui↔OmniOne bridge, USDC/USDT→ooKRW 전환, 실제 원화 상환, zkLogin 인프라 신규 구축은 이 선택 작업에도 포함하지 않는다. Sui까지 필수라는 결정이 나오면 HK-06 담당/계약/시연 기준을 별도로 확정하고 추가 인력 또는 다른 범위 축소를 합의한다. “시간이 남으면 bridge까지”는 허용하지 않는다.
...
- [ ] 지정 장소/캠페인/정책 및 M0/M1/S 범위 승인.
- [ ] CX/OpenDID/Chain 실제 smoke와 지원 담당자 확보.
...
- [ ] A01–A12 결과·실제/테스트 주입 여부·미해결 사항 기록.
...
- [ ] 실제 CX 결과·VC issue/holder ack·VP 검증·redemption·chain receipt의 비식별 evidence 묶음.
- [ ] 정상+거절/중복/복구 영상, MVP 시연 영상과 결선 제안서. 실제 연결/목업/제외 기능 표.
- [ ] 실제 기관 서비스가 아닌 ONDO 체험 혜택이라는 설명, 남은 연동과 종료 후 테스트 데이터 삭제 담당.

**녹화 권장 구성(내부 3분 예시, 공식 제한 아님):** 지도·혜택 소개 20초 → 실제 CX/holder·VC/VP 70초 → 사용과 같은 장소 복귀 30초 → 중복·거절 30초 → 비식별 Chain receipt·실제/목업 구분 30초. 상세 개인정보·QR secret·token 화면은 가리며 실패 장면을 샘플 성공 영상으로 덮지 않는다.

완료 판정은 “M0 실제 연결, M1 실제 연결, 서비스 시연, 복구·보안, 제출 자료”를 각각 체크한다. mock 테스트 826개 등 과거 수치는 이번 실제 연동 PASS로 재사용하지 않는다. **이 문서의 모든 새 연동 체크박스는 개발 전 미완료 상태다.**

## 11. 개발자가 첫 회신에 채울 양식

```text
개발 인원 / 담당: A=Harvey(BE·Chain, 수락 여부 ___), B=woogieboogie 측(FE·QA, 담당 ___), C=추가 지원 필요 여부 ___
CX / OpenDID / OmniOne 환경: 준비됨(증거 ___) / 미준비(담당·해결기한 ___)
실제 holder / 시연 OS·기기 / 언어: ___
지정 장소 / 캠페인 / 정책 만료: ___
M0·M1 범위: 수락 / 변경 요청(사유·대안 ___)
Sui: 이번 범위 제외. 변경 요청 시 별도 승인·담당·추가일정 ___
