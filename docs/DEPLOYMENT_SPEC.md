...
- 원 인계 시작점: branch `handoff/harvey-20260914`의 `k-tour-id-app/`. 현재 브랜치는 위 시작 안내를 따르며 과거 배포 commit ID는 이력 참고값이다.
...
- 원 기준선의 standalone HTTP route는 `/`, `/ondo-b`(redirect), `/api/ondo/venues/[venueId]`이다. 현재 별도 Sandbox에는 `/api/kyc/sumsub/session`(POST/DELETE), `/api/kyc/sumsub/status`(GET) 두 route가 추가되어 있다. `scripts/ondo-b-standalone/policy.mjs`가 `/partner/verify`, `/partner/settlements`, `/evidence`, `/onboarding`, `/wallet`, `/pass`, `/present` 등 legacy route를 배포에서 차단한다.
