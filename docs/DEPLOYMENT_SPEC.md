...
**현재 공개 주소는 [운영 앱](https://ondo-tau.vercel.app)** 이다. source/runtime `e2ad7c4`, [운영 앱](https://ondo-tau.vercel.app), [고유 배포 주소](https://ondo-eq9z4oba8-jaewook-9643s-projects.vercel.app), production deployment `dpl_435M4FCDrrR8AWGgNhmCTD9J3g8J` Ready. 로컬 전체 계약833/833(11.3초)·typecheck/build/scan·HTTP probe·After 19 공개 경로5/5(2.7분) PASS. 새 운영 HTTP 및 After 19 공개 경로5/5(2.0분, workers1/retries0) PASS. 5개 project mismatch skip은 실행 수에 포함하지 않는다. URL·검사/복구 기준은 [매장 After 19 겹침 수정 릴리스](./PLACE_AFTER19_FIX_2026-09-14.md)을 따른다. 이전 `82ea4c9`의 공개 mobile12개/desktop·tablet2개 및 계약826개는 [이전 지도·지갑 릴리스](./MAP_WALLET_JOURNEYS_2026-09-12.md)의 역사적 증거이며 새 source에서 재실행한 것으로 합산하지 않는다. [9월 11일 전체 검수](./FINAL_JOURNEY_QA_2026-09-11.md)도 별도다. 실제 provider 연결·실기기 완료가 아니다.
...
- 현재 목업·배포 source/runtime: `e2ad7c4`, production deployment `dpl_435M4FCDrrR8AWGgNhmCTD9J3g8J` Ready. [매장 After 19 겹침 수정 릴리스](./PLACE_AFTER19_FIX_2026-09-14.md)과 [이전 지도·지갑 릴리스](./MAP_WALLET_JOURNEYS_2026-09-12.md) 및 [이전 전체 여정 검수](./FINAL_JOURNEY_QA_2026-09-11.md)를 source·URL별로 구분한다.
...
| 식당 예약/부분환불·별도 authorize/capture | 샘플 구현 및 기존 f79 기준선 commerce 검수 PASS. 현재 `e2ad7c4`의 실행 범위·제한은 [매장 After 19 겹침 수정 릴리스](./PLACE_AFTER19_FIX_2026-09-14.md)을 따르며 [9월 11일 검수](./FINAL_JOURNEY_QA_2026-09-11.md)와 합산하지 않는다. 실제 예약·주문 processor와 금전 원장은 별도 backend 필요 |
...
| 모바일·adversarial·최종 공유 URL QA | `e2ad7c4` production Ready. 로컬 전체 계약833/833(11.3초)·typecheck/build/scan·HTTP probe·After 19 공개 경로5/5(2.7분) PASS. 새 운영 HTTP 및 After 19 공개 경로5/5(2.0분, workers1/retries0) PASS. 5개 project mismatch skip은 실행 수에 포함하지 않는다. [매장 After 19 겹침 수정 릴리스](./PLACE_AFTER19_FIX_2026-09-14.md)에 수정 전 운영 재현·로컬/운영 실행·로그를 보존한다. 이전 `82ea4c9`의 공개 mobile12개/desktop·tablet2개 및 계약826개는 [이전 지도·지갑 릴리스](./MAP_WALLET_JOURNEYS_2026-09-12.md)의 역사적 증거이며 새 source에서 재실행한 것으로 합산하지 않는다. 실제 iPhone Safari/Android·provider handoff는 미검증 |

이 문서에는 완료율을 부여하지 않는다. 상태는 해당 commit의 재현 가능한 증거가 생긴 뒤에만 갱신한다.

## Appendix A. G01–G13 인계 추적 요약

이 표는 기존 FL/REQ를 유지한 compact trace다. 경로는 `k-tour-id-app/` 기준이며 표의 `B/`는 `features/ondo/`를 뜻한다. 현재 source/runtime `e2ad7c4`의 공개 주소·deployment·After 19 겹침 수정 검사는 [매장 After 19 겹침 수정 릴리스](./PLACE_AFTER19_FIX_2026-09-14.md)을 따른다. 이전 `82ea4c9` 결과는 [이전 지도·지갑 릴리스](./MAP_WALLET_JOURNEYS_2026-09-12.md)에 보존한다. G01/G02 진입의 `d9eda4f` 결과는 [지도 우선 진입 기록](./MAP_FIRST_ENTRY_2026-09-14.md)의 역사적 증거다. 아래 G별 상세 검수는 **역사적** [9월 11일 전체 여정 검수](./FINAL_JOURNEY_QA_2026-09-11.md)의 프리뷰 기준선174개·수정 영향27개다. 각 행의 “현재/최신”은 그 기록 당시 범위이며 새 운영 URL 실행으로 합산하지 않는다. `52f376e` 운영 증거는 [이전 운영 기록](./PRODUCTION_RELEASE_2026-09-12.md)에 보존한다. 최초 실패와 재실행 조건을 제외하고 모두 첫 실행 PASS로 읽지 않는다. 관련 test 열은 공개 `/` suite와 계약의 소스 연결점이며 파일 존재 자체가 실행 또는 PASS 증거는 아니다. `3dc392b`는 [9월 10일 릴리스](./PROTOTYPE_COMPLETION_2026-09-10.md), `a45400f`·`996119f`·`5233816`/v3.2도 역사적 기록이다. FE gap은 프런트·provider handoff 관점의 남은 표면이며 backend 작업 전체를 반복하지 않는다.
...
- [x] `release:current`: source/runtime `e2ad7c4` / production deployment `dpl_435M4FCDrrR8AWGgNhmCTD9J3g8J` Ready. [매장 After 19 겹침 수정 릴리스](./PLACE_AFTER19_FIX_2026-09-14.md)의 URL·완료된 검사 범위를 따른다. 실제 provider 완료와 별도다
...
- [x] `qa:public`: **공개 실행 범위 한정 PASS** — `e2ad7c4` 운영 HTTP probe와 After 19 공개 경로5/5(2.0분, workers1/retries0) PASS. 5개 project mismatch skip은 실행 수에 포함하지 않는다. [매장 After 19 겹침 수정 릴리스](./PLACE_AFTER19_FIX_2026-09-14.md)에서 최종 상태를 확인하며 로컬 PASS를 원격 PASS로 바꾸지 않는다. 이전 `82ea4c9`의 공개 mobile12개/desktop·tablet2개 및 계약826개는 [이전 지도·지갑 릴리스](./MAP_WALLET_JOURNEYS_2026-09-12.md)의 역사적 증거이며 새 source에서 재실행한 것으로 합산하지 않는다. [9월 11일 검수](./FINAL_JOURNEY_QA_2026-09-11.md)도 별도다
